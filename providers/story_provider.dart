...
import 'package:Taillz/utills/utils.dart';
...
class StoryProvider with ChangeNotifier {
  ...
  int get pageNumberForData => _pageNumberForData;
 ...
 Future<List<CommentModel>?> getReplayComments(
      {required BuildContext context,
      int? commentID,
      int? pageNumber,
      String? authTOKEN}) async {
    Uri uri = Uri.tryParse(
      '${ApiNetwork.getRepliedComments}$commentID?page=$pageNumber&limit=10',
    )!;
    WidgetProvider widgetProvider = Provider.of<WidgetProvider>(
      context,
      listen: false,
  ...
  Future disableComments(
      {required BuildContext context, required StoryModel story,required bool allow}) async {
    Uri uri = Uri.tryParse(
    ...
    await widgetProvider
      ...
      BotToast.showText(
        text: allow?TKeys.enable_toast_text.translate(context):TKeys.disable_toast_text.translate(context),
        contentColor: Constants.blueColor,
        textStyle: const TextStyle(
          color: Colors.white,
          fontSize: 12,
